...
// Inject styles into document (guarded so the module can load during SSR)
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  // Controlled mode: an external `value` prop overrides internal input state
  const inputValue = props.value ?? input;
  const setInputValue = props.onValueChange ?? setInput;
  ...
  const handleSubmit = () => {
    if (inputValue.trim() || files.length > 0) {
      ...
      const formattedInput = messagePrefix ? `${messagePrefix}${inputValue}]` : inputValue;
