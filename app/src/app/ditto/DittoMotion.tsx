...
type RTReveal = {
  anchor: string; opacity: string; transform: string; transition: string;
...
export default function DittoMotion({ spec }: { spec: MotionSpec }) {
  useEffect(() => {
    ...
    // A horizontal ticker only makes sense on a row-oriented track; at mobile widths the
    // same <ul> renders as a static column and translating it would push it off-screen.
    const isColumnTrack = (el: HTMLElement) => getComputedStyle(el).flexDirection.startsWith("column");

    for (const w of spec.waapi) {
      const el = byDittoId(w.anchor);
      if (!el) continue;
      if (w.iterations < 0 && w.keyframes.some((k) => typeof k.transform === "string") && isColumnTrack(el)) continue;
      try {
